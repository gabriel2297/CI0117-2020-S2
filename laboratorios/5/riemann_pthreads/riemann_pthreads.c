...
double riemman_sum(double a, double b, double n, double delta){
    double result = 0.0;
...
int main(int argc, char * argv[]){
	...
	/* Moviendo el calculo del delta afuera de la funcion ya que es constante */
	double delta = (b - a) / n;
	if (delta < 0){
		delta = -1 * delta;
	}
    ...
    clock_t begin = clock();
    double result = riemman_sum(a, b, n, delta);
    clock_t end = clock();
